...
def cases_page(
    ...
    return templates.TemplateResponse(
        request,
        "cases.html",
        {"cases": cases, "active": "cases", "current_user": current_user},
    )
...
def new_case_page(
    ...
    return templates.TemplateResponse(
        request,
        "case_new.html",
        {"case_types": CASE_TYPES, "active": "cases", "current_user": current_user},
    )
...
async def upload_evidence(
    ...
    return RedirectResponse(f"/cases/{case.id}/evidence?uploaded={created}", status_code=303)
...
@app.post("/cases/{case_id}/evidence/{item_id}/update")
def update_evidence(
    case_id: int,
    item_id: int,
    db: Annotated[Session, Depends(get_db)],
    ...
    return RedirectResponse(f"/cases/{case.id}/evidence", status_code=303)
...
@app.post("/cases/{case_id}/imports/document-organizer")
...
@app.post("/cases/{case_id}/imports/email")
...
@app.get("/cases/{case_id}/insights")
def insights_page(
    request: Request,
    case_id: int,
    db: Annotated[Session, Depends(get_db)],
    ...
    return templates.TemplateResponse(
        request,
        "insights.html",
        {"case": case, "insight": insight, "active": "cases", "current_user": current_user},
    )


@app.post("/cases/{case_id}/insights/generate")
def generate_insights(
    ...
    return RedirectResponse(f"/cases/{case.id}/insights", status_code=303)
...
def export_page(
    ...
    return templates.TemplateResponse(
        request,
        "export.html",
        {"case": case, "packages": packages, "active": "cases", "current_user": current_user},
    )
